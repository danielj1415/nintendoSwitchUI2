  ...
  useEffect(() => {
    const handleKeyDown = (event) => {
      if (event.key === "ArrowRight") {
        ...
        setSelectedIcon((prev) => (prev + 1) % iconData.length); // Move to the next game
      } else if (event.key === "ArrowLeft") {
        event.preventDefault(); // Prevent the default horizontal page scroll
        setSelectedGame((prev) => (prev - 1 + squares.length) % squares.length); // Move to the previous game
        setSelectedIcon((prev) => (prev - 1 + iconData.length) % iconData.length); // Move to the previous game
      }
    };
